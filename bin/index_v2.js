...
async function main() {
  if(fs.existsSync(path.join(process.cwd(), "config.json"))){
    let { confirmLoadConfigFile } = await inquirer.prompt([
      {
        type: 'confirm',
        name: 'confirmLoadConfigFile',
        message:
          'config file was found. Do you want to load config file?',
      },
    ]);

    if(confirmLoadConfigFile){
...
async function getBasePath() {
  ...
  if (base_path === 0) {
    ...
    while(wrongInput){
      ...
      if(path.isAbsolute(file_location)){
...
async function generateImages() {
  ...
  if (config.deleteDuplicates) {
    while (!Object.values(weightedTraits).filter(arr => arr.length == 0).length && numberOfNFT<=totalFilesNumber) {
      let picked = [];
      order.forEach(id => {
...
async function rarityCalculate(){
  for(key in metaData){
    ...
    for(var i = 0; i < metaData[key]['attributes'].length; i++){
      let traitValue = metaData[key]['attributes'][i]['value'] +".png";
      metaData[key]['attributes'][i]['rarity_score'] = (1/ (elementSummary[traitValue]['generated'] / totalNFT)).toFixed(2);
...
async function writeMetadata() {
  ...
  { 
    ...
    if(overidingMetadataFile){
      ...
      await writeFile(path.join(metadata_output_dir,'metadata.json'), JSON.stringify(metaData), { flag: 'a+' },  err => {});
    }
...
async function writeConfig() {
  await writeFile(path.join(basePath,'config.json'), JSON.stringify(config, null, 2));
}
